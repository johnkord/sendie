...
export class CryptoService {
   ...
   * This gives users a memorable identity (e.g., "cosmic-tiger")
   */
  async generateFriendlyName(publicKeyJwk: string): Promise<string> {
    ...
    // Use first two bytes: one for adjective (mod 64), one for noun (mod 64)
    const adjective = ADJECTIVES[hashArray[0] % ADJECTIVES.length];
    const noun = NOUNS[hashArray[1] % NOUNS.length];
